...
$con=mysql_connect("localhost","root","123");//连接mysql 数据库，账户名root ，密码root
if (!$con) {
    die('数据库连接失败'.$mysql_error());
}
mysql_select_db("14",$con);//use user_info数据库；
$dbusername=null;
$dbpassword=null;
$result=mysql_query("select * from user where user ='{$username}' and isdelete =0;");//查出对应用户名的信息，isdelete表示在数据库已被删除的内容
